...
class SDocNode(SDocNodeIF):
    ...
    def __init__(
        ...
        # cache to lessen the performance impact as this is accessed from multiple jinja templates 
        self._parsed_discussion_cache: Optional[dict[str, DiscussionThread]] = None
    ...
    def discussion(self) -> Optional[str]:
        return self._get_cached_field(
            RequirementFieldName.DISCUSSION, singleline_only=False
        )

    @property
    def parsed_discussion(self) -> Optional[dict[str, DiscussionThread]]:
        if self._parsed_discussion_cache is not None:
            return self._parsed_discussion_cache

        if not self.discussion or not self.discussion.strip():
            self._parsed_discussion_cache = None
            return None

        self._parsed_discussion_cache = DiscussionParser.parse(self.discussion)
        return self._parsed_discussion_cache
    ...
    def set_field_value(
        ...
        *,
        ...
        if field_name == RequirementFieldName.DISCUSSION:
            self._parsed_discussion_cache = None
